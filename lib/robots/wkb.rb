class WKB
  ...
  # Constants
  unless self.const_defined?(:FIRE_MIN)
    FIRE_MIN = 0
    FIRE_MAX = 3
    TURN_MIN = -10
    TURN_MAX = 10
  ...
  class Heading
    ...
    def ==(other)
      (value - other.value) < 0.001
    end

    def delta_to(other)
      (179 + other.value - value) % 360 - 179
    end
  end
  ...
  class Scan
    ...
    def range(my_x, my_y)
      Math.hypot(my_x - @x, my_y - @y)
    end
  end

  # Targets class - Holds a history of scanned targets
  class Targets
    unless self.const_defined?(:MAX_TICKS)
      MAX_TICKS = 10
      BULLET_SPEED = 30
    end
    def initialize
  ...
  class Movement
    unless self.const_defined?(:ANGLE_TOWARDS)
      ANGLE_TOWARDS = 75
      ANGLE_AWAY = 105
      WAIT_TIME = 30
    end
  ...
  # Utilities
  def heading
    Heading.new(state[:heading])
  end

  def gun_heading
    Heading.new(state[:gun_heading])
  end

  def radar_heading
    Heading.new(state[:radar_heading])
  end
