...
class DuckBill09
  ...
  def tick(events)
    ...
    @sincehit += 1
    @sincehit = 0 unless events['got_hit'].empty?
    @sinceevade += 1
    # +++++  Tracking +++++ #
    ...
    if @runmode == 'rotate'
      ...
      if walls.sort.first < 100 && @mytrack.range < @chase_lim[1]
        @runmode = 'runstreight'
        @since_evade = 0
      elsif @mytrack.range > @chase_lim[1]
        @runmode = 'rotate'
      ...
      if (time + @time_sync) % @period == 0
        @dir = -@dir
      elsif @hit_filter > 6 && @sinceevade > 20
        @sinceevade = 0
