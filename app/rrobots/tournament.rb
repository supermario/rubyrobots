...
class Match
  ...
  def winner_points
    return 0.5 if simul?
    winner_health / (winner_health + loser_health)
  end

  def loser_points
    return 0.5 if simul?
    loser_health / (winner_health + loser_health)
  end
...
class Round
  ...
  def initialize(matches)
    ...
    stats_to_init = %w(wins points ties margin simul timedout round_wins)
...
def print_round_table(stat, rounds, ranking, _round_tables, html)
  ...
  rows.each do |row|
    ...
    cols.each do |col|
      round = nil
      if row != col
        round = rounds.find { |x| x.bots.key?(row) && x.bots.key?(col) }
...
def to_html(match_data, html)
  ...
  stats = %w(wins points round_wins margin simul timedout ties)
  rankings = {}
...
ARGV.grep(/^-matches=(\d+)/)do |item|
  matches_per_round = Regexp.last_match[1].to_i
  ARGV.delete(item)
end
...
ARGV.grep(/^-(.*)$/)do |_item|
  puts "Unknown option '-#{Regexp.last_match[1]}'"
  puts
  usage
end
...
suffix = 0
while File.exist?('tournament'  + suffix.to_s.rjust(2, '0') + '.yml')
  suffix = suffix + 1
end
